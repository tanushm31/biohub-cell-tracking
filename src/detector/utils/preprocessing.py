...
def gaussian_blur(
    volume: np.ndarray,
    sigma: tuple[float, float, float] = (0.5, 1.0, 1.0),
) -> np.ndarray:
    """
    Apply Gaussian smoothing.
    """

    return gaussian_filter(volume, 
        sigma=sigma
    )
...
def preprocess(
    ...
    volume = gaussian_blur(
        volume,
        sigma=(
            config.gaussian_sigma_z,
            config.gaussian_sigma_y,
            config.gaussian_sigma_x,
        ),
    )
